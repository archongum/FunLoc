...
//                XposedBridge.log("FL: in getLastLocation! Caller package name: $packageName")
         ...
         */
        MethodFinder.fromClass(clazz)
            .filterByName("handleLocationChangedLocked")
            .filterPublic()
            .forEach { method -> method.createHook {
                    before { param ->
                        XposedBridge.log("FL: in handleLocationChangedLocked (R)! Removing whitelisted apps...")
                        val mRecordsByProviderField = FieldFinder.fromClass(clazz)
                            .filterByName("mRecordsByProvider")
                            .first()
                        ...
                        records.entries.forEach { entry ->
                            ...
                            value.forEach { record ->
                                val mReceiver = FieldFinder.fromClass(record.javaClass)
                                    .filterByName("mReceiver")
                                    .first().get(record)
                                val mCallerIdentity = FieldFinder.fromClass(mReceiver.javaClass)
                                    .findSuper()
                                    .filterByName("mCallerIdentity")
                                    .first().get(mReceiver)
                                ...
                                if (!ConfigGateway.get().inWhitelist(packageName)) {
                                    ...
                                    MethodFinder.fromClass(mReceiver.javaClass)
                                        .filterByName("callLocationChangedLocked")
                                        .filterPublic()
                                        .first()
...
//            XposedBridge.log("FL: in requestGeofence (R)! Caller package name: $packageName")
    ...
    fun hookDLC(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        MethodFinder.fromClass(clazz)
            .filterByName("initializeProvidersLocked")
            .filterPrivate()
            .forEach { method -> method.createHook {
                after { param ->
